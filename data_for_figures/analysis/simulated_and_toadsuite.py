...
Acknowledgements
---------------
Thanks to Lena De Framond for running the simulated audio files on the TOADSuite!
...
ax0 = plt.subplot(131,projection='3d')
...
plt.plot(distance_from_mic0, tristar_relative_error,'.')
plt.ylabel('Relative error, $\\frac{Localisation\ error}{Distance\ from\ central\ mic} $', fontsize=12);
plt.xlabel('Point distance from central mic, m', fontsize=12)
plt.xticks(fontsize=10);plt.yticks(fontsize=10)
plt.text(0.1, 0.9, "C", transform=ax13.transAxes, fontsize=15)
plt.tight_layout()
...
# distance from mic 0 of the cave array
delta_xyz_mic0 = cave_mics_xyz[0,:] - cave_points
dist_from_mic0 = delta_xyz_mic0.apply(calculate_euclidean_error, 1)
relative_error = cave_euclidean_error/dist_from_mic0

plt.figure(figsize=(12,4))
ax2 = plt.subplot(131, projection='3d')
...
ax3 = plt.subplot(132)
plt.plot(dist_from_mic0,cave_euclidean_error, '.')
plt.ylabel('Error, m', fontsize=12);
plt.xlabel('Point distance from tristar central mic, m', fontsize=12)
plt.xticks(fontsize=10);plt.yticks(fontsize=10)
plt.text(0.1,0.9,'B', transform=ax3.transAxes, fontsize=15)
plt.tight_layout()

ax4 = plt.subplot(133)
plt.plot(dist_from_mic0,relative_error, '.')
plt.ylabel('Relative error, $\\frac{Localisation\ error}{Distance\ from\ central\ mic} $', fontsize=12);
plt.xlabel('Point distance from tristar central mic, m', fontsize=12)
plt.xticks(fontsize=10);plt.yticks(fontsize=10)
plt.text(0.1,0.9,'C', transform=ax4.transAxes, fontsize=15)
plt.tight_layout()

print(f'95%ile relative errors: {np.percentile(relative_error, [2.5,97.5]) }')
